...
must_haves:
  ...
  artifacts:
    ...
    - path: "package.json"
      provides: "npm script for E2E test orchestration"
      contains: "test:e2e"
  key_links:
...
<task type="auto">
  ...
  **Add npm scripts to `package.json`:**
  ...
  "test:e2e": "npm run test:e2e:seed && npm run test:e2e:run"
  ```

  The `test:e2e` script is the single-command entry point: it seeds AsyncStorage, then runs all Maestro flows. The app must already be running on an iOS simulator (`npx expo start --dev-client`) -- document this requirement.

  **Note:** Do NOT add `wait-on` or Expo start/stop to the npm script. The dev server should be started separately. This avoids complexity with process management and keeps the script focused on seed + test.

  Also add a comment in the seed script about the prerequisite: "Requires: iOS simulator booted, app installed via `eas build --profile development-simulator --platform ios --local`, Expo dev server running."
  </action>
  <verify>
  ...
  - `cat package.json | grep "test:e2e"` shows the orchestration scripts
  - YAML syntax is valid: `python3 -c "import yaml; yaml.safe_load(open('.maestro/flows/habit-creation.yaml'))"` (or visual inspection)
  </verify>
  <done>
  Three standalone Maestro flows exist covering habit creation (guided builder end-to-end), habit completion (tap card, skip evidence), and weekly review (fill form, submit). Each flow captures screenshots at key checkpoints. npm scripts provide `test:e2e` as single-command entry point.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  **Prerequisites (one-time setup):**
  ...
  4. Start Expo dev server: `npx expo start --dev-client`
  ...
  **Expected behavior:**
  1. Seed script populates AsyncStorage with test data
  2. Maestro runs 3 flows sequentially:
  ...
  3. Screenshots captured at checkpoints in `~/.maestro/tests/`
  4. All 3 flows report PASSED
  ...
  **If flows fail:**
  ...
  <done>All 3 E2E flows (habit-creation, habit-completion, weekly-review) execute successfully against a running Expo development build on iOS simulator. Screenshots captured at key checkpoints for visual confirmation.</done>
</task>
...
- `package.json` has `test:e2e` script that seeds and runs
- Flows execute against a running Expo development build
- Human confirms all 3 flows pass on iOS simulator
</verification>

<success_criteria>
`npm run test:e2e` executes seed script then runs all 3 Maestro flows against a running development build. Habit creation flow completes guided builder and verifies new habit on Today screen. Habit completion flow taps a habit card and skips evidence. Weekly review flow fills the review form and submits. All 3 flows report PASSED with screenshots captured at checkpoints.
</success_criteria>
